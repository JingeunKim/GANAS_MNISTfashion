...
class Pop():
    ...
    def initialization_pop(self):
        ...
        for pop in range(self.population):
            ...
            while (cnt < self.pop_layer - 1):
                num = np.random.rand()
                if num < 0.05:  # conv layer +batch normalization 5%
                    num = 3
                elif num < 0.3 and num >= 0.05:  # SENet block 25%
                    num = 2
                elif num >= 0.3 and 0.45 > num:  # pooling layer 15%
                    num = 3
    ...
    def evolve(self, device, population, conv_unit):
        ...
        for Chromosome in range(params_settings.population):
