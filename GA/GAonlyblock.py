...
class Pop():
    ...
    def initialization_pop(self):
        ...
        for pop in range(self.population):
            ...
            while cnt < self.pop_layer - 1:
                ...
                if num < 0.25:  # SENet block 25%
                    num = 2
                elif 0.45 > num >= 0.25:  # pooling layer 20%
                    num = 3
                elif 0.45 <= num < 0.7:  # resnet block 25%
                    num = 6
                elif 0.7 <= num < 0.95:  # densenet block 25%
                    num = 7
                else:  # dense 5%
                    num = 4

                # if num < 0.05:  # conv layer +batch normalization 0%
                #     num = 3
                # elif num < 0.3 and num >= 0.05:  # SENet block 25%
                #     num = 2
                # elif num >= 0.3 and 0.45 > num:  # pooling layer 20%
                #     num = 3
                # elif num >= 0.45 and num < 0.7:  # resnet block 25%
                #     num = 6
                # elif num >= 0.7 and num < 0.95:  # densenet block 25%
                #     num = 7
                # else:  # dense 5%
                #     num = 4
                ...
                appendlayer = [2, 6, 7]
                if arr[cnt] == 3 and arr[cnt + 1] == 3 or arr[1] == 3:
                    arr[cnt + 1] = random.choice(appendlayer)
                cnt += 1

            if arr[len(arr) - 1] == 2 or arr[len(arr) - 1] == 3 or arr[
    ...
    def crossover(self, layer_parent1, layer_parent2, unit_parent1, unit_parent2):
        ...
        if len_layer_parent1 >= len_layer_parent2:
            cross_pt = len_layer_parent2 // 2 + 1  # np.random.randint(0, len_layer_parent2)
        else:
            cross_pt = len_layer_parent1 // 2 + 1  # np.random.randint(0, len_layer_parent1)
        if cross_pt == 2:
            cross_pt = 1
    ...
    def evolve(self, device, population, conv_unit):
        ...
        for ch in range(len(population) // 4):
            ...
            layer_child1, layer_child2, unit_child1, unit_child2, selection = self.select(all_error, population,
                                                                                          conv_unit, selection)
        ...
        torch.save(model, f"./" + params_settings.save_model_name + ".pt")
        print("end..")
    ...
    def drawGA(self, value):
        ...
        plt.ylabel("Fitness")
        plt.plot(value)
        plt.savefig('./' + params_settings.save_model_name + '_generation.png')
        plt.show()
